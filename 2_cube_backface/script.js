...
function fillFace(face, proj) {
	...
	const a_proj = proj(vs[face[0]]);
	ctx.moveTo(a_proj.x, a_proj.y);
	for (let i = 1; i < face.length; i++) {
		const v = proj(vs[face[i]]);
		ctx.lineTo(v.x, v.y);
	}
	ctx.closePath();
	ctx.fill();
...
function is_front_face(face, sight_vec) {
	const [p1, p2, p3] = face;
	...
	if (dot_product(sight_vec, N) > 0) {
		return true;	// The surface is back-facing
	}
	return false;
}
...
function frame() {
	...
	let proj_3d = (x) => translate_z(rotate_zy(rotate_xz(x, angle_y), angle_x), dz);
	let proj = (x) => screen(project(proj_3d(x)));
	...
	for (const face of fs) {
		const face_proj = face.map((i) => proj_3d(vs[i]));

		for (let i = 0; i < face.length; i++) {
			const a_proj = proj(vs[face[i]]);
			const b_proj = proj(vs[face[(i + 1) % face.length]]);

			line(a_proj, b_proj, colors="purple");
		}
		if (is_front_face(face_proj, sight_vec)) {
			fillFace(face, proj);
		}
	}
}
...
frame();
// document.addEventListener("click", (e) => { frame() });
